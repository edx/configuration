...
def rds_controller(rds_list, username, password, hostname, dd_apikey, dd_appkey, port, indexname, environment):
    for item in rds_list:
        ...
        if len(global_str) > 0:
            # Datadog initialization with your API credentials
            options = {
                'api_key': dd_apikey
                'app_key': dd_appkey
            }
            ...
            api.Event.create(
                    ...
                    text=global_str,  # your log data
                    tags=[f"env:{environment}", f"index:{index}", "source:INNODB", "db:rds"]
                )
...
@click.option('--dd_app_key', envvar='DDAPPKEY', required=True)
...
def main(username, password, environment, hostname, dd_apikey, dd_appkey, indexname, rdsignore, whitelistregions):
    rds_list = rds_extractor(environment, whitelistregions)
    filtered_rds_list = list([x for x in rds_list if x['name'] not in rdsignore])
    rds_controller(filtered_rds_list, username, password, hostname, dd_apikey, dd_appkey, indexname, environment)
